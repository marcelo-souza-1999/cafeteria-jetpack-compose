...
class ViaCepDataSource(
    ...
    suspend fun fetchAddressByCep(cep: String): Result<ViaCepResponse> {
        return try {
            ...
            val response: ViaCepResponse = httpClient.get("https://viacep.com.br/ws/$sanitizedCep/json/").body()
    ...
    suspend fun searchCepByAddress(uf: String, city: String, street: String): Result<List<ViaCepResponse>> {
        return try {
            ...
            val response: List<ViaCepResponse> = httpClient.get("https://viacep.com.br/ws/$sanitizedUf/$sanitizedCity/$sanitizedStreet/json/").body()
            Result.success(response)
        } catch (e: IOException) {
            Result.failure(e)
        } catch (e: SerializationException) {
